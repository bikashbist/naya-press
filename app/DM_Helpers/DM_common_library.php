...
{
    $lib = new \App\Model\Dcms\NepaliCalender();
    $a = explode("-", $date);
    $b = explode(" ", $a[2]);
...
{
    $nepcal = new \App\Model\Dcms\NepaliCalender();
...
{
    $data['applicant'] = DB::table('memberships')->where("user_id", Auth::user()->id)->where('status', '=', 1)->first();
