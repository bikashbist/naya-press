...
class Province extends DM_BaseModel
{
    /** One to many Relationship between Posts and Category */
    public function campuses() {
        $lang_id = DM_Post::setLanguage();
        return $this->hasMany(Campus::class, 'province_id', 'id')->where('status', '=', 1)->where('lang_id', '=', $lang_id);
    }

    /** One to many Relationship between Posts and Category */
    public function affiliatedCampuses() {
        $lang_id = DM_Post::setLanguage();
        return $this->hasMany(Campus::class, 'province_id', 'id')->where('status', '=', 1)->where('campus_type', '=', 'affiliated')->where('lang_id', '=', $lang_id);
    }

    public function constituentCampuses() {
        $lang_id = DM_Post::setLanguage();
        return $this->hasMany(Campus::class, 'province_id', 'id')->where('status', '=', 1)->where('campus_type', '=', 'constituent')->where('lang_id', '=', $lang_id);
    }

     /** Category Tree */
     public function categoryTree() {
        $data['rows'] = Province::orderBy('order')->get();
        $ref = [];
        $items = [];
            ...
            $html.= '<li class="dd-item dd3-item" data-id="'.$value['id'].'">
                        ...
                        <div class="dd3-content"><span id="label_show'.$value['id'].'">'.$value['name'].'</span>
                            <span class="span-right">
                                ...
                                &nbsp;&nbsp;
                                <a class="btn btn-warning" id="'.$value['id'].'" label="'.$value['name'].'" href="\dashboard/province/'. $value['id'].'/edit" ><i class="fa fa-pencil"></i></a>
