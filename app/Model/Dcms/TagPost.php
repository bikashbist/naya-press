...
class TagPost extends DM_BaseModel
{
    use SoftDeletes;
    /**
    ...
    protected $folder = 'tag_post';
    protected $prefix_path_image = '/upload_file/images/tag_post/';
    protected $prefix_path_file = '/upload_file/files/tag_post/';
    ...
    public function storeData(Request $request, $tag, $type, $rows, $image, $status, $file_title, $files)
    {
    ...
    public function updateData(Request $request, $type, $rows, $image, $tag, $status, $file_title, $files, $post_unique_id)
    {
        // for thumbnail
        if ($request->hasFile('images')) {
            $post_thumbnail = parent::uploadMultipleImages($request, $this->folder_path_image, $this->prefix_path_image, 'images');
        ...
        foreach ($rows as $row) {
            if (isset($row['id'])) {
                $post = TagPost::findOrFail($row['id']);
            } else {
                $post = new TagPost;
