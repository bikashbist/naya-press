...
@section('content')
    <div class="container-fluid mt-2">
        ...
        <div class="container p-0">
            <div class="row mt-4">
                <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 d-flex gap-2 "
                    ...
                    <span>
                        @if ($data['lang'] == 1)
                            {{ timesAgoEn($data['row']->created_at, true) }}
                        @else
                            {{ timesAgoNp($data['row']->created_at, true) }}
                        @endif
                    </span>
                    <i class="fa-solid fa-calendar pt-1"></i>
                    <span>
                        @if ($data['lang'] == 1)
                            {{ date('F d, Y', strtotime($data['row']->created_at)) }}
                        @else
                            {{ datenep($data['row']->created_at, true) }}
                        @endif
                    </span>
                    <i class="fa-solid fa-eye pt-1"></i>
                    <span>
                        @if ($data['lang'] == 1)
                            Visted:{{ $data['row']->visit_no }}
                        @else
                            हेरिएका:{{ getUnicodeNumber($data['row']->visit_no, true) }}
                        @endif
                    </span>
                </div>
            </div>
            <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12 d-flex justify-content-center mt-5"
                style="color:#1c1ca1;">
                <h2 class="fw-bold detail-heading2 m-0">
        ...
        @php
            $url = route('site.post.show', ['post' => $data['row']->post_unique_id]);
        @endphp
        <div class="news-detail-social-share-icons flex-row align-items-center" style="margin-left: 100px;">
            ...
            <div class="news-detail-social-share-item news-detail-social-share-facebook">
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($url) }}" class="st-custom-button"
                    target="_blank">
            ...
            <div class="news-detail-social-share-item news-detail-social-share-twitter">
                <a href="https://twitter.com/intent/tweet?url={{ urlencode($url) }}&text={{ $data['row']->title }}"
                    class="st-custom-button" target="_blank">
            ...
            <div class="news-detail-social-share-item news-detail-social-share-whatsapp">
                <a href="https://api.whatsapp.com/send?text={{ urlencode($url) }}" class="st-custom-button"
                    target="_blank">
